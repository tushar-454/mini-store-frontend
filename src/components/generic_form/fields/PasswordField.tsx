import { PasswordMessage } from '@/components/generic_form/fields/password-helpers/PasswordMessage';
import { PasswordStrength } from '@/components/generic_form/fields/password-helpers/PasswordStrength';
...
export const PasswordField = <T extends FieldValues>({
  ...
  const { showPassword, setShowPassword, strength, checks } = usePasswordField(password);
      ...
      render={({ field }) => (
        <FormItem className={cn(className)}>
          ...
          <div className='space-y-2'>
            ...
            {showMessage && <PasswordMessage requirements={requirements} checks={checks} />}
          </div>
...
const usePasswordField = (password: string) => {
  ...
  const [checks, setChecks] = useState<boolean[]>(new Array(requirements.length).fill(false));
